...
long_metabar <- read.csv("Data/Processed/genera_long_23.csv") #or long_metabar <- read.csv("data/processed/genera_adelgidae_long_23.csv")
...
colnames(reads_per_sample)<- paste0("ASV",seq_along(colnames(reads_per_sample)))
...
Joined_agg <- Joined_agg[,c(1,5,4)]
Joined_agg <- Joined_agg %>% aggregate(DailyCount ~ unique_id + genus, FUN=sum)
colnames(Joined_agg)[1] <-"date"

#### Calculate all stats for no filter ####
percentage_common <- congruence_find(long_metabar,Joined_agg)


write.csv(percentage_common,"Data/Processed/congruence_table.csv",row.names = FALSE)


#### Calculate all stats for 0.1% filter ####
sample_01_perc_spe <- reads_join[reads_join$measure >=(0.01*reads_join$Total_reads),]

percentage_common_1 <- congruence_find(sample_01_perc_spe,Joined_agg)
percentage_common_1$total_met <- percentage_common_1$Common + percentage_common_1$only_met

nrow(percentage_common_1[percentage_common_1$total_met > percentage_common_1$total_morph,])
...
percentage_common_05 <- congruence_find(sample_05_perc_spe,Joined_agg)
percentage_common_05$total_met <- percentage_common_05$Common + percentage_common_05$only_met
nrow(percentage_common_05[percentage_common_05$total_met > percentage_common_05$total_morph,])




#### Calculate all stats for 0.02% filter ####
sample_02_perc_spe <- reads_join[reads_join$measure >=(0.002*reads_join$Total_reads),]
percentage_common_02 <- congruence_find(sample_02_perc_spe,Joined_agg)
...
## Plot figure 2: (top,bottom)
svg(file="Figures/Congruence_genera_detect.svg", width=6.7,height=5.2)
...
svg(file="Figures/Congruence_genera_jaccard.svg", width=6.7,height=5.2)
...
#### Calculate false positives and negatives at genus level
...
#  geom_boxplot(width=0.7,alpha=0.7,position=position_dodge(1))+
#  scale_fill_viridis(discrete = TRUE,option="viridis")+theme_bw()


### Plot Supplementary 
svg(file="Figures/Errors_genera.svg", width=6.5,height=5.3)
...
congruence_anim <- congruence[congruence$method == "congruence",]
meta_data <-  read.csv("Data/Raw/Joined.aphids.cleaned_23.csv")
meta_data <- unique(meta_data[,c(1,6)])
colnames(meta_data)[1] <- "Sample"

congruence_anim <- inner_join(congruence_anim,meta_data,by="Sample")
write.csv(congruence_anim,"Data/Processed/animated_info_congruence.csv")

all_controls_anim <- inner_join(all_controls,meta_data,by="Sample")
write.csv(all_controls_anim,"Data/Processed/all_controls.csv")
...
mean(false_negative01$n)
mean(false_negative03$n)


# Mean false positives after filter
mean(false_positive01$n)
mean(false_positive03$n)
