...
Joined_agg <- read.csv("Data/Processed/removed_species_long.csv")
...
drop_na(congruence[congruence$method=="congruence",]) %>%
  group_by(group) %>% 
  summarise(mean(percent))

drop_na(congruence[congruence$method=="jaccard",]) %>%
  group_by(group) %>% 
  summarise(mean(percent))
...
drop_na(all_controls) %>%
  group_by(group,filter) %>% 
  summarise(mean(n))

write.csv(all_controls,"Data/Processed/all_controls_removed_species.csv",row.names = F)
